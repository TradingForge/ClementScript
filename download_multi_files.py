#!/usr/bin/env python3
"""
Download historical data files from Betfair Historical Data service using multiple processes.
This script downloads football data files from Betfair's free plan with multi-process support.
Work is divided by month across processes.
"""
...
def get_month_from_date(date_str):
    """Extract month number (1-12) from date string."""
    try:
        if not date_str:
            return None
...
def should_process_month(month, process_id, total_processes):
    """Determine if this process should handle this month based on the division pattern."""
    if month is None:
        return False
    
    # X=1: Process all months
    if total_processes == 1:
        return True
    
    # X=2: Process 1: 1,3,5,7,9,11; Process 2: 2,4,6,8,10,12
    elif total_processes == 2:
        if process_id == 1:
            return month in [1, 3, 5, 7, 9, 11]
        else:  # process_id == 2
            return month in [2, 4, 6, 8, 10, 12]
    
    # X=3: Process 1: 1,4,7,10; Process 2: 2,5,8,11; Process 3: 3,6,9,12
    elif total_processes == 3:
        if process_id == 1:
    ...
    # X=4: Process 1: 1,5,9; Process 2: 2,6,10; Process 3: 3,7,11; Process 4: 4,8,12
    elif total_processes == 4:
        if process_id == 1:
            return month in [1, 5, 9]
        elif process_id == 2:
            return month in [2, 6, 10]
        elif process_id == 3:
            return month in [3, 7, 11]
        else:  # process_id == 4
...
def download_worker(process_id, total_processes, football_data):
    """Worker function for each process to download files for assigned months."""
    print(f"\n{'='*70}")
    print(f"Process {process_id} of {total_processes} started")
    print(f"{'='*70}")
    
    # Connect to Betfair for this process
    trading = connect_to_betfair()
    
    # Filter football_data for this process based on month
    filtered_data = []
    for item in football_data:
        date = item.get('forDate', 'N/A')
        month = get_month_from_date(date)
        
        if should_process_month(month, process_id, total_processes):
            filtered_data.append(item)
    
    print(f"\nProcess {process_id}: Handling {len(filtered_data)} items")
    ...
    for item in filtered_data:
        ...
        print(f"\nProcess {process_id} - Processing: {sport} - {date} (ID: {purchase_item_id})")
    ...
    print(f"\n{'='*70}")
    print(f"Process {process_id} Summary")
    print(f"{'='*70}")
...
def main():
    """Main function to download historical data using multiple processes."""
    print("=" * 70)
    print("Betfair Historical Data Download Script (Multi-Process)")
    print("=" * 70)
    ...
    # Connect to Betfair (main process for initial data fetch)
    trading = connect_to_betfair()
    ...
    print(f"\nStarting {NUM_PROCESSES} download process(es)...")
    
    if NUM_PROCESSES == 1:
        # Single process - just run directly
        download_worker(1, 1, football_data)
    else:
        # Multiple processes
        processes = []
        for i in range(1, NUM_PROCESSES + 1):
            p = Process(target=download_worker, args=(i, NUM_PROCESSES, football_data))
            p.start()
            processes.append(p)
